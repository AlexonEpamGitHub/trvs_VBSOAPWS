...
namespace SOAPWebServices.Core.Infrastructure
{
    ...
    public static class ApplicationEvents
    {
        ...
        public static void ConfigureApplicationEvents(WebApplication app, ILogger? logger = null)
        {
            ...
            // Call individual event methods to maintain separation of concerns
            OnApplicationStarting(app, logger);
            
            // Request pipeline middleware
            ConfigureRequestPipeline(app, logger);
            
            // Error handling
            ConfigureErrorHandling(app, logger);
            
            // Register application stopping events
            OnApplicationStopping(app, logger);
        }

        /// <summary>
        /// Handles functionality that would typically be in Application_Start event
        /// </summary>
        public static void OnApplicationStarting(WebApplication app, ILogger? logger = null)
        {
            logger?.LogInformation("Application starting: Initializing core components");
            ...
            // Log current GC mode
            logger?.LogInformation("GC Configuration: Server GC={ServerGC}, Concurrent={ConcurrentGC}, Generation 0={Gen0Size}, " +
            ...
            // Initialize any legacy application state or caches here
            logger?.LogInformation("Initial application setup completed successfully");
        }

        /// <summary>
        /// Configures error handling to replace Application_Error event
        /// </summary>
        public static void OnError(WebApplication app, ILogger? logger = null)
        {
            logger?.LogInformation("Configuring global error handling to replace Application_Error");
            
            // This method is called by ConfigureErrorHandling
        ...
        /// <summary>
        /// Handles functionality for application shutdown (Application_End equivalent)
        /// </summary>
        public static void OnApplicationStopping(WebApplication app, ILogger? logger = null)
        {
            logger?.LogInformation("Registering application stopping event handlers");
            ...
            if (lifetime != null)
            {
                ...
                {
                    logger?.LogInformation("Application is stopping - executing graceful shutdown tasks");
                    ...
                    try
                    {
                        ...
                        logger?.LogInformation("Flushing pending operations and closing resources");
                        
                        // Allow time for in-flight requests to complete
                        Task.Delay(TimeSpan.FromSeconds(2)).Wait();
                        
                        // Close/dispose any application resources here
                        logger?.LogInformation("Resource cleanup completed successfully");
                    }
                    catch (Exception ex)
                    {
                    ...
                    {
                        logger?.LogInformation("Shutdown sequence completed");
                    }
                });
            }
        }
        ...
        /// <summary>
        /// Configures the request pipeline middleware to handle BeginRequest, EndRequest events
        /// </summary>
        private static void ConfigureRequestPipeline(WebApplication app, ILogger? logger = null)
        {
            // Replace Application_BeginRequest and Application_EndRequest
            app.Use(async (context, next) =>
            {
        ...
        /// <summary>
        /// Configures global error handling to replace Application_Error event
        /// </summary>
        private static void ConfigureErrorHandling(WebApplication app, ILogger? logger = null)
        {
            OnError(app, logger);
        }
    }
