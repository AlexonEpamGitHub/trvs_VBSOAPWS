...
/// </summary>
[DataContract]
[XmlType("ResponseDtoBase")]
public record ResponseDtoBase : IValidatableObject
{
    /// <summary>
    ...
    /// <summary>
    /// Operation identifier for request tracking
    /// </summary>
    [DataMember]
    ...
    /// <summary>
    /// Validates the base response object
    /// </summary>
    public virtual IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        ...
        {
            results.Add(new ValidationResult("Status is required", [nameof(Status)]));
        }
...
/// <summary>
/// Base response data transfer object for SOAP operations (maintaining backward compatibility)
/// </summary>
[DataContract]
[XmlType("Response")]
...
public record SampleDataResponse : ResponseDtoBase
{
    /// <summary>
...
/// <summary>
/// Report data item extending sample data item with report-specific properties
/// </summary>
[DataContract]
[XmlType("ReportDataItem")]
public record ReportDataItem : SampleDataItem
{
    /// <summary>
...
public record ReportDataResponse : ResponseDtoBase
{
    /// <summary>
...
/// <summary>
/// Factory class for creating response DTOs to replace legacy DataSet usage
/// </summary>
public static class ResponseDtoFactory
{
    /// <summary>
    /// Creates a sample data response with default sample items to match legacy DataSet behavior
    /// </summary>
    /// <param name="operationId">Operation identifier for tracking</param>
    /// <param name="collectionName">Name of the data collection</param>
    /// <param name="dataSource">Source system identifier</param>
    /// <returns>SampleDataResponse with sample data items</returns>
    public static SampleDataResponse CreateSampleDataResponse(string? operationId = null, string? collectionName = "SampleData", string? dataSource = "DefaultSource")
    {
        var sampleItems = new List<SampleDataItem>
        {
            new() {
                Id = 1,
                Name = "Sample Item 1",
                Description = "First sample data item",
                Value = "Sample Value 1",
                DataType = "string",
                Category = "General",
                Tags = ["sample", "test", "data"],
                Properties = [
                    new KeyValueItem { Key = "Type", Value = "Standard", ValueType = "string" },
                    new KeyValueItem { Key = "Priority", Value = 1, ValueType = "int" }
                ],
                Source = dataSource,
                Version = "1.0"
            },
            new() {
                Id = 2,
                Name = "Sample Item 2",
                Description = "Second sample data item",
                Value = 42,
                DataType = "int",
                Category = "Numeric",
                Tags = ["sample", "numeric"],
                Properties = [
                    new KeyValueItem { Key = "Type", Value = "Numeric", ValueType = "string" },
                    new KeyValueItem { Key = "Precision", Value = 0, ValueType = "int" }
                ],
                Source = dataSource,
                Version = "1.0"
            },
            new() {
                Id = 3,
                Name = "Sample Item 3",
                Description = "Third sample data item",
                Value = true,
                DataType = "bool",
                Category = "Boolean",
                Tags = ["sample", "boolean"],
                Properties = [
                    new KeyValueItem { Key = "Type", Value = "Boolean", ValueType = "string" },
                    new KeyValueItem { Key = "Default", Value = false, ValueType = "bool" }
                ],
                Source = dataSource,
                Version = "1.0"
            }
    ...
    /// <summary>
    /// Creates a report data response with default report items to match legacy behavior
    /// </summary>
    /// <param name="reportId">Report identifier</param>
    /// <param name="reportName">Report name or title</param>
    /// <param name="requestedBy">User who requested the report</param>
    /// <returns>ReportDataResponse with report data items</returns>
    public static ReportDataResponse CreateReportDataResponse(string reportId, string reportName, string? requestedBy = null)
    {
        var reportItems = new List<ReportDataItem>
        {
            new() {
                Id = 1,
                Name = "Report Metric 1",
                Description = "First report metric",
                Value = 150.75m,
                DataType = "decimal",
                ...
                Metadata = [
                    new KeyValueItem { Key = "CalculatedAt", Value = DateTime.UtcNow, ValueType = "datetime" },
                    new KeyValueItem { Key = "Formula", Value = "SUM(values)", ValueType = "string" }
                ]
            },
            new() {
                Id = 2,
                Name = "Report Metric 2",
                Description = "Second report metric",
                Value = 95.5m,
                DataType = "decimal",
                Category = "Quality",
                Section = "Details",
                ...
                Metadata = [
                    new KeyValueItem { Key = "CalculatedAt", Value = DateTime.UtcNow, ValueType = "datetime" },
                    new KeyValueItem { Key = "Formula", Value = "AVG(values)", ValueType = "string" }
                ]
            }
        };
        ...
        {
            ReportType = "Standard",
            Description = "Standard report with sample metrics",
            Parameters = [
                new KeyValueItem { Key = "DateRange", Value = "Last 30 days", ValueType = "string" },
                new KeyValueItem { Key = "IncludeInactive", Value = false, ValueType = "bool" }
            ],
            DateRangeStart = DateTime.UtcNow.AddDays(-30),
            DateRangeEnd = DateTime.UtcNow,
            Source = "ReportingService",
            Version = "2.0",
            SuccessCount = reportItems.Count,
            ErrorCount = 0,
            WarningCount = 0,
            Metrics = [
                new MetricItem {
                    ...
                    Value = 125.5m,
                    Unit = "ms",
                    Description = "Report processing time",
                    Category = "Performance",
    ...
    /// <summary>
    /// Creates an empty sample data response for error scenarios
    /// </summary>
    /// <param name="errorMessage">Error message</param>
    /// <param name="operationId">Operation identifier</param>
    /// <returns>Empty SampleDataResponse with error information</returns>
    public static SampleDataResponse CreateEmptySampleDataResponse(string errorMessage, string? operationId = null)
    {
        return new SampleDataResponse
        {
            Status = "Error",
    ...
    /// <summary>
    /// Creates an empty report data response for error scenarios
    /// </summary>
    /// <param name="reportId">Report identifier</param>
    /// <param name="reportName">Report name</param>
    /// <param name="errorMessage">Error message</param>
    /// <returns>Empty ReportDataResponse with error information</returns>
    public static ReportDataResponse CreateEmptyReportDataResponse(string reportId, string reportName, string errorMessage)
    {
        return new ReportDataResponse
        {
            Status = "Error",
    ...
    /// <summary>
    /// Creates a paginated sample data response
    /// </summary>
    /// <param name="pageNumber">Page number</param>
    /// <param name="pageSize">Page size</param>
    /// <param name="totalCount">Total item count</param>
    /// <param name="operationId">Operation identifier</param>
    /// <returns>Paginated SampleDataResponse</returns>
    public static SampleDataResponse CreatePaginatedSampleDataResponse(int pageNumber, int pageSize, int totalCount, string? operationId = null)
    {
        ...
        {
            var itemIndex = startIndex + i + 1;
            sampleItems.Add(new SampleDataItem
            {
                Id = itemIndex,
                Name = $"Sample Item {itemIndex}",
                Description = $"Sample data item number {itemIndex}",
                Value = $"Value {itemIndex}",
                DataType = "string",
                Category = "Generated",
                Tags = ["sample", "paginated"],
                CreatedAt = DateTime.UtcNow.AddMinutes(-itemIndex)
            });
        }

        var hasMorePages = (pageNumber * pageSize) < totalCount;

        return new SampleDataResponse
        {
            Status = "Success",
            Message = $"Retrieved page {pageNumber} of sample data",
            OperationId = operationId,
            SampleItems = sampleItems,
